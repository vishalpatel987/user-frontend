...
const Register = () => {
  ...
  const onSubmit = async (data) => {
    try {
      await api.post('/auth/register', data); // ✅ sahi API call
      toast.success('Registration successful');
      navigate('/login');
    } catch (err) {
      toast.error(err.response?.data?.message || 'Registration failed');
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      ...
      >
        ...
        <input {...register('password', { required: true, minLength: 6 })} type="password" placeholder="Password" className="border p-2 rounded" />
        <button type="submit" className="bg-green-600 text-white p-2 rounded hover:bg-green-500 transition">Register</button>
        <Link to="/login" className="text-sm text-green-500 hover:underline text-center">Login</Link>
      </form>
    </div>
  );
};
